          ...
          <TableBody>
            {isLoading ? (
              ...
              >
                ...
                <TableCell className="text-center">
                  {inq.status === 'new' ? (
                    <Badge className="bg-yellow-400 text-black border-none uppercase text-[9px] font-extrabold px-3 py-1">Nou</Badge>
                  ) : inq.status === 'replied' ? (
          ...
          {selectedInquiry && (
            <div className="py-10 space-y-12">
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6 bg-neutral-50 p-8 rounded-[2.5rem]">
              ...
              <section className="space-y-6">
                ...
                <div className="bg-neutral-900 p-8 rounded-[2.5rem] text-white flex items-center justify-between shadow-xl">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-accent-lime"><Package size={24} /></div>
                    <div>
                      <p className="font-headline font-extrabold text-lg uppercase">{selectedInquiry.productName}</p>
                      <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">ID Produs: {selectedInquiry.productId.slice(0, 8)}</p>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <div className="pt-10 border-t border-neutral-50 space-y-4">
                <Link href={`/admin/oferta/${selectedInquiry.id}`}>
                  <Button className="w-full bg-neutral-900 hover:bg-black text-white font-extrabold h-16 rounded-3xl flex items-center justify-between pl-10 pr-2 group shadow-2xl shadow-black/10">
                    {selectedInquiry.status === 'replied' ? 'VEZI / MODIFICĂ OFERTA' : 'GENEREAZĂ OFERTA'}
                ...
                {selectedInquiry.status === 'replied' && (
                  <div className="text-center p-6 bg-neutral-50 rounded-[2rem] border border-neutral-100">
